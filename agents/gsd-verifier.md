...
gsd-remix-sdk query roadmap.get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(gsd-remix-sdk query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `gsd-remix-sdk query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(gsd-remix-sdk query verify.artifacts "$PLAN_PATH")
```
...
Use `gsd-remix-sdk query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(gsd-remix-sdk query verify.key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(gsd-remix-sdk query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(gsd-remix-sdk query verify.commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(gsd-remix-sdk query roadmap.analyze --raw)
```
