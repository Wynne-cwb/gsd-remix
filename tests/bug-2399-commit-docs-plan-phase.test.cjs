...
describe('plan-phase commit_docs support (#2399)', () => {
  ...
  test('plan-phase.md uses gsd-remix-sdk query commit for the plan commit', () => {
    const content = fs.readFileSync(PLAN_PHASE_PATH, 'utf-8');
    // Must use gsd-remix-sdk query commit (not raw git) so commit_docs guard in gsd-tools is respected
    assert.ok(
      content.includes('gsd-remix-sdk query commit') || content.includes('gsd-tools') || content.includes('gsd-remix-sdk'),
      'plan-phase.md plan commit step must use gsd-remix-sdk query commit (not raw git commit)'
    );
  });
});
