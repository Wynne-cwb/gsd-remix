...
- Read frontmatter `status` field via:
  ```bash
  gsd-remix-sdk query frontmatter.get .planning/threads/{file} status 2>/dev/null
  ```
...
2. Update the thread file's frontmatter `status` field to `resolved` and `updated` to today's ISO date:
   ```bash
   gsd-remix-sdk query frontmatter.set .planning/threads/{SLUG}.md status resolved
   gsd-remix-sdk query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
   ```

3. Commit:
   ```bash
   gsd-remix-sdk query commit "docs: resolve thread — {SLUG}" ".planning/threads/{SLUG}.md"
   ```
...
```bash
gsd-remix-sdk query frontmatter.set .planning/threads/{SLUG}.md status in_progress
gsd-remix-sdk query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
```
...
1. Generate slug from description:
   ```bash
   SLUG=$(gsd-remix-sdk query generate-slug "$ARGUMENTS" --raw)
   ```
...
5. Commit:
   ```bash
   gsd-remix-sdk query commit "docs: create thread — ${ARGUMENTS}" ".planning/threads/${SLUG}.md"
   ```
...
- Status fields read via gsd-remix-sdk query frontmatter.get — never eval'd or shell-expanded
- The generate-slug call for new threads runs through gsd-remix-sdk query (or gsd-tools) which sanitizes input — keep that pattern
</security_notes>
