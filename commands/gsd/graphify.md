...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-remix-sdk query`. Use `node $HOME/.claude/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-remix-sdk query` where a handler exists.
