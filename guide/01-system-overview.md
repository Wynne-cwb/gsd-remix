...
workflow 里大量出现的 `gsd-remix-sdk query ...`，说明设计正在从“bash + cjs 工具集合”逐渐迁移到“可编程 query registry + runner”。
...
flowchart TD
    ...
    E --> F["Task / AskUserQuestion / gsd-remix-sdk query"]
...
- agent 文件是“角色职责说明书”。
- `gsd-remix-sdk query` 和 `bin/lib/*.cjs` 才是很多真实状态变更的落点。
- `.planning/` 是它真正的长期记忆区。
