...
**Follow-up:** confirm **`GOLDEN_PARITY_EXCEPTIONS`** for any remaining read-only registry gaps (`learnings.query`, `progress.bar` — still exception-only until strict rows); extend **`read-only-golden-rows.ts`** when aligned.
...
3. **Track D — CJS deprecation headers** (done)
   - `gsd-tools.cjs` already has `@deprecated` JSDoc header (lines 3-6) pointing to `gsd-remix-sdk query` and `@gsd-remix/sdk`.
...
- **Upstream PRs:** `Wynne-cwb/gsd-remix` issue #2302.
...
## Reference pattern: porting a read-only handler (e.g. `audit-uat`)

These are fixed by **aligning the TypeScript handler with the CJS implementation**, then adding a row to `READ_ONLY_JSON_PARITY_ROWS`.

1. **Find the CJS source of truth**  
   - `audit-uat`: `get-shit-done/bin/lib/uat.cjs` → `cmdAuditUat`  
   - `gsd-tools.cjs` `runCommand` switch shows the top-level command and argv.

2. **Implement or adjust the SDK module**  
   - Example: `auditUat` in `sdk/src/query/uat.ts` mirrors `cmdAuditUat` (`parseUatItems`, milestone filter, `summary.by_*`), throws the same error text as CJS where applicable, and returns `data` matching the CJS JSON shape.
...
**When not to copy line-for-line:** subprocess-only or volatile concerns (e.g. clock-derived fields differing between subprocess and in-process runs). Then **normalize in the test** (see `audit-open`: strip volatile `scanned_at` before `toEqual`)—**document in QUERY-HANDLERS.md**, do not delete the assertion.
...
| ~~8~~ | ~~**`init.*` read-only JSON**~~ | various | `init.cjs` / `init-complex` | `init.ts`, `init-complex.ts` | **Done:** `golden.integration.test.ts` + nine init composition tests; `withProjectRoot` / `subagent_timeout` / `GOLDEN_INTEGRATION_MAIN_FILE_CANONICALS`; see `QUERY-HANDLERS.md`. |

**Suggested order:** Audit/read-only batch above is complete — follow-ups via **`GOLDEN_PARITY_EXCEPTIONS`** / new strict rows as needed (`learnings.query`, `progress.bar`, etc.).
...
**Done (claude-md + milestone mutation batch):** `generate-claude-md` (`claude-md.ts`); `milestone.complete` (`phase-lifecycle.ts` + `readModifyWriteStateMdFull`); `GOLDEN_MUTATION_SUBPROCESS_COVERED` updated; **`MUTATION_SUBPROCESS_GAP_REASON` removed** from `golden-policy.ts`.
