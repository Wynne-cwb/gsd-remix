...
```bash
npm install @gsd-remix/sdk
```

## Quickstart — programmatic

```typescript
import { GSD, createRegistry } from '@gsd-remix/sdk';
...
```bash
node ./node_modules/@gsd-remix/sdk/dist/cli.js query state.json
node ./node_modules/@gsd-remix/sdk/dist/cli.js query roadmap.analyze
```
...
| Area | Entry |
|------|--------|
| Query registry | `createRegistry()` in `src/query/index.ts` — same handlers as `gsd-remix-sdk query` |
| Tools bridge | `GSDTools` — native dispatch with optional CJS subprocess fallback |
| Orchestrators | `PhaseRunner`, `InitRunner`, `GSD` |
| CLI | `gsd-remix-sdk` — `query`, `run`, `init`, `auto` |
