  ...
  if (!candidate) {
    return {
      check: {
        ...
        fix: 'Reinstall GSD (npx get-shit-done-cc@latest) to restore the bundled bridge, or run /gsd-health --runtime --repair for SDK CLI repair.',
      },
      path: null,
      source: 'missing',
      available: false,
  ...
  try {
    ...
    if (!output) {
      return {
        check: {
          ...
          fix: 'Reinstall GSD (npx get-shit-done-cc@latest) to refresh the bridge assets before relying on fallback commands.',
          path: candidate.path,
    ...
    return {
      check: {
        ...
        detail,
        fix: 'Reinstall GSD (npx get-shit-done-cc@latest) to refresh bundled assets, then retry the workflow.',
        path: candidate.path,
        source: candidate.source,
      },
      path: candidate.path,
