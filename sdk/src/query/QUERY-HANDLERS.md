...
| `runtime.health` / `runtime health` / `sdk.health` / `sdk health` | Deterministic GSD runtime self-check. Validates the current Node version against the installed package `engines.node` requirement and probes the `gsd-tools.cjs` bridge used by CJS fallback commands. Returns `passed`, `blockers`, `warnings`, `gsd_tools_source`, and `legacy_bridge_available` so workflows can fail fast on broken runtime installs before deeper orchestration starts. |
