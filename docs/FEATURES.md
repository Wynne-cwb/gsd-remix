...
**Plan Checker Verification (7 Dimensions):**
...
**Commands:** `/gsd-complete-milestone`, `/gsd-new-milestone [name]`

**Purpose:** Archive milestone, tag release, and start the next development cycle.
...
**Config:** `model_profile: <quality|balanced|budget|inherit>` (set via `/gsd-settings` or `.planning/config.json`)
...
**Requirements:**
- REQ-HOOK-01: Statusline MUST display model, directory, context usage, and runtime-provided rate limits
...
**Statusline Display:**
```
model │ directory [█████░░░░░ 50%] [5h:75%@14:30] [7d:40%]
```
...
**Part of:** `/gsd-progress` (cross-phase health check)
...
When verification returns `human_needed`, items are persisted as a trackable HUMAN-UAT.md file with `status: partial`. Feeds into the cross-phase health check.
...
**Commands:** `/gsd-add-backlog <description>`, `/gsd-review-backlog`

**Purpose:** Capture ideas that aren't ready for active planning. Backlog items use 999.x numbering to stay outside the active phase sequence.
...
- REQ-PLAY-03: System MUST capture and compare visual state against phase expectations
...
**Purpose:** Allow projects to store their CLAUDE.md in a non-root location. The `claude_md_path` config key controls where GSD commands read and write the generated CLAUDE.md file.

**Requirements:**
- REQ-CMDPATH-01: `claude_md_path` defaults to `./CLAUDE.md`
- REQ-CMDPATH-02: Commands read the path from config and write to the specified location
