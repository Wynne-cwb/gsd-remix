...
[![GitHub stars](https://img.shields.io/github/stars/Wynne-cwb/gsd-remix?style=for-the-badge&logo=github&color=181717)](https://github.com/Wynne-cwb/gsd-remix)
...
控制每个代理使用哪个 Claude 模型。所有命名配置（`quality`/`balanced`/`budget`）现在解析为同一套统一分配：规划、路线图、研究和调试类代理使用 Opus，其余代理使用 Sonnet。`inherit` 则跟随当前会话模型。

| 配置 | 规划/研究/调试 | 其余代理 |
|---------|----------|-----------|
| `quality` / `balanced` / `budget` | Opus | Sonnet |
| `inherit` | 会话模型 | 会话模型 |
...
OpenCode、Gemini CLI、Kilo 和 Codex 现在通过 `npx gsd-remix` 原生支持。
...
<a href="https://star-history.com/#Wynne-cwb/gsd-remix&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Wynne-cwb/gsd-remix&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Wynne-cwb/gsd-remix&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Wynne-cwb/gsd-remix&type=Date" />
 </picture>
</a>
