...
4. **执行角色层**: `agents/*.md`
5. **工具/查询层**: `gsd-sdk query` 和 `get-shit-done/bin/gsd-tools.cjs`
...
flowchart LR
  ...
  C --> W["workflows/*.md"]
  W --> A["agents/*.md"]
  W --> Q["gsd-sdk query / gsd-tools.cjs"]
  Q --> P[".planning/ 状态文件"]
  P -. 状态被读取与回写 .-> W
...
3. `process`
4. 分步骤说明
5. 每一步尽量通过 `gsd-sdk query` 获取结构化数据

GSD 很少让 orchestrator 自己去手写一堆 ad-hoc Bash 逻辑, 而是倾向于:

- 先 `gsd-sdk query init.*`
- 再基于 JSON 做分支
...
- `gsd-sdk query agent-skills gsd-planner`
- `gsd-sdk query config-get ...`
...
- `gsd-sdk query milestone.complete "vX.Y" --name "..."`
...
- `gsd-sdk query init.phase-op <phase>`
...
- `gsd-sdk query todo.match-phase <N>`
...
## 9. Query 层: workflow 为什么总在调用 `gsd-sdk query`
...
> `gsd-sdk query` 是主路, `gsd-tools.cjs` 是兼容层和回退层。
...
- `gsd-sdk query init.plan-phase 1`
...
- `gsd-sdk query roadmap.annotate-dependencies`
...
### 误区 6: “`gsd-sdk` 和 `gsd-tools.cjs` 是重复的”
