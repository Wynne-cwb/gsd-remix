...
| `workflow.team_mode` | string | `off` | Team mode for `/gsd-autonomous`: `off` (default — inline loop), `auto` (use Agent-team coordination when the runtime + a no-op Agent probe pass, else silently fall back to inline), or `on` (require it — error if unavailable). Front-loads all human decisions (Decision Harvest), runs a fresh teammate per bounded step, and defers UAT to one end-of-milestone packet. Staged opt-in: default flips to `auto` once proven. See `references/team-mode.md`. |
