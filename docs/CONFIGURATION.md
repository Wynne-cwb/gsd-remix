...
| `workflow.team_mode` | string | `auto` | Team mode for `/gsd-autonomous`: `auto` (default — use agent-team coordination when a multi-agent-capable runtime (Claude Code or Codex) + a no-op spawn probe pass, else silently fall back to inline), `off` (force the inline loop even on a capable runtime), or `on` (require it — error if unavailable). Front-loads all human decisions (Decision Harvest), runs a fresh teammate per bounded step, and defers UAT to one end-of-milestone packet. On Codex it runs single-level (Lead is sole spawner, teammates are leaves). `auto` is safe by construction — it never leaves a half-state on probe failure. See `references/team-mode.md`. |
| `workflow.team_max_parallel` | number | `3` | Max concurrent teammate leaves the Lead spawns at once in team mode. The Lead batches fan-out to this cap and releases each finished teammate before spawning the next, then falls back to inline if spawning is unavailable. Keeps Codex fan-out under its (schema-unspecified) concurrency limit; raise it once a runtime's real cap is known. See `references/team-mode.md`. |
| `workflow.auto_milestone` | string | `ask` | After `/gsd-new-project` / `/gsd-new-milestone` approves a ROADMAP, hand off to `/gsd-autonomous` (Team Lead runs the whole milestone). Capability-gated — only when the runtime is multi-agent-capable (Claude Code or Codex), `workflow.team_mode` ≠ `off`, and a no-op spawn probe passes; otherwise falls back to the normal "next step" messaging. `ask` (default — confirm once, then hand off on yes), `auto` (hand off seamlessly, no confirm), or `off` (never — always stop and guide). Headless/`--auto` skips the confirm. Clears the per-phase auto-advance chain so autonomous is the sole driver. See `references/milestone-autopilot.md`. |
