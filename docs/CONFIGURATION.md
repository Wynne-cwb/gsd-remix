...
| `workflow.auto_milestone` | string | `ask` | After `/gsd-new-project` / `/gsd-new-milestone` approves a ROADMAP, hand off to `/gsd-autonomous` (Team Lead runs the whole milestone). Capability-gated — only when the runtime is Claude Code, `workflow.team_mode` ≠ `off`, and a no-op Agent probe passes; otherwise falls back to the normal "next step" messaging. `ask` (default — confirm once, then hand off on yes), `auto` (hand off seamlessly, no confirm), or `off` (never — always stop and guide). Headless/`--auto` skips the confirm. Clears the per-phase auto-advance chain so autonomous is the sole driver. See `references/milestone-autopilot.md`. |
