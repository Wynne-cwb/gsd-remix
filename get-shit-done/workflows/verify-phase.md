...
```bash
gsd-remix-sdk query roadmap.get-phase "${phase_number}"
...
Use `gsd-remix-sdk query` verify handlers (or legacy gsd-tools) to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(gsd-remix-sdk query frontmatter.get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(gsd-remix-sdk query roadmap.get-phase "${phase_number}" --raw)
```
...
Use `gsd-remix-sdk query verify.artifacts` (or legacy gsd-tools) for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(gsd-remix-sdk query verify.artifacts "$plan")
...
Use `gsd-remix-sdk query verify.key-links` (or legacy gsd-tools) for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(gsd-remix-sdk query verify.key-links "$plan")
