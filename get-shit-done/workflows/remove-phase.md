...
```bash
INIT=$(gsd-remix-sdk query init.phase-op "${target}")
...
**Delegate the entire removal operation to `gsd-remix-sdk query phase.remove`:**

```bash
RESULT=$(gsd-remix-sdk query phase.remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), the CLI will error. Use `--force` only if the user confirms:

```bash
RESULT=$(gsd-remix-sdk query phase.remove "${target}" --force)
```
...
```bash
gsd-remix-sdk query commit "chore: remove phase {target} ({original-phase-name})" .planning/
```
...
- Don't manually renumber — use `gsd-remix-sdk query phase.remove` which handles all renumbering
...
- [ ] `gsd-remix-sdk query phase.remove` executed successfully
