...
```bash
DETAIL=$(gsd-remix-sdk query roadmap.get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(gsd-remix-sdk query init.phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(gsd-remix-sdk query config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
gsd-remix-sdk query commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" "${phase_dir}/${padded_phase}-CONTEXT.md"
```
