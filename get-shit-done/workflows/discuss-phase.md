...
- Per-project: `gsd-remix-sdk query config-set workflow.text_mode true`
...
AGENT_SKILLS_ADVISOR=$(gsd-remix-sdk query agent-skills gsd-advisor 2>/dev/null)
```
...
```bash
TODO_MATCHES=$(gsd-remix-sdk query todo.match-phase "${PHASE_NUMBER}")
```
...
```bash
MAX_PASSES=$(gsd-remix-sdk query config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
gsd-remix-sdk query commit "docs(${padded_phase}): capture phase context" "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
gsd-remix-sdk query commit "docs(state): record phase ${PHASE} context session" .planning/STATE.md
```
</step>
