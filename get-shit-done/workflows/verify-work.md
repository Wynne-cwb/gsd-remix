...
```bash
INIT=$(gsd-remix-sdk query init.verify-work "${PHASE_ARG}")
...
```
UI_PHASE_FLAG=$(gsd-remix-sdk query config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(gsd-remix-sdk query uat.render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
gsd-remix-sdk query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
`audit-open` is CJS-only until registered on `gsd-remix-sdk query`:

```bash
gsd-remix-sdk query audit-open --json 2>/dev/null
```
