...
```bash
INIT=$(gsd-remix-sdk query docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(gsd-remix-sdk query agent-skills gsd-doc-writer 2>/dev/null)
```
...
```bash
gsd-remix-sdk query commit "docs: generate project documentation" \
