...
```bash
INIT=$(gsd-remix-sdk query init.milestone-op)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_CHECKER=$(gsd-remix-sdk query agent-skills gsd-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(gsd-remix-sdk query resolve-model gsd-integration-checker --raw)
```
...
gsd-remix-sdk query phases.list
```
...
PHASE_INFO=$(gsd-remix-sdk query find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  gsd-remix-sdk query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(gsd-remix-sdk query config-get workflow.nyquist_validation --raw 2>/dev/null)
```
