...
AGENT_SKILLS_UI=$(gsd-remix-sdk query agent-skills gsd-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(gsd-remix-sdk query agent-skills gsd-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(gsd-remix-sdk query resolve-model gsd-ui-researcher --raw)
UI_CHECKER_MODEL=$(gsd-remix-sdk query resolve-model gsd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(gsd-remix-sdk query config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
```bash
gsd-remix-sdk query commit "docs(${padded_phase}): UI design contract" "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
