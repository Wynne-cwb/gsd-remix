...
2. Config value: `gsd-remix-sdk query config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(gsd-remix-sdk query config-get workflow.code_review_depth 2>/dev/null || echo "")
